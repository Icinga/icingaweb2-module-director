...
class BasketSnapshotCustomVariableTest extends BaseTestCase
{
    ...
    public function testRestoreClearsAWholeStoredValueWhenTheRootItselfIsRetyped(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $db = $this->getDb();
        [$host, $property] = $this->createTemplateWithProperty($db);
        $property->set('value_type', 'string');
        $property->store();

        $host->vars()->set(self::PROP_KEY_NAME, 'seattle');
        $host->store();
        ...
        // A string no longer fits once the root becomes a number, this stale
        // value has to be cleared, same as a retyped nested field would be.
        $decoded = json_decode($json);
        $decoded->CustomVariable->{$propertyUuidString}->value_type = 'number';
        ...
        $this->assertNull(
            $restoredHost->vars()->get(self::PROP_KEY_NAME),
            'a root value that no longer fits its retyped schema must be cleared, not left stale'
        );
    }

    public function testRestoreKeepsARootValueThatAlreadyFitsAfterARetype(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $host->vars()->set(self::PROP_KEY_NAME, 'some string');
        $host->store();
        ...
        // This host already carries its value in the new shape, the same way a
        // host restored earlier in this same basket would. The retype must not
        // wipe it out just because the schema's type string changed.
        $decoded = json_decode($json);
        $decoded->CustomVariable->{$propertyUuidString}->value_type = 'number';
        $decoded->HostTemplate->{self::TEMPLATE_NAME}->vars->{self::PROP_KEY_NAME} = 42;
        ...
        $this->assertEquals(
            42,
            $restoredHost->vars()->get(self::PROP_KEY_NAME)->getValue(),
            'a value that already fits the new type must survive the retype'
        );
    }
