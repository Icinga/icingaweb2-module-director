...
class DirectorPropertyTest extends BaseTestCase
{
    ...
    public function testReservedOverrideHandoffKeyIsRejectedAsAPropertyName(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $property = DirectorProperty::create([
            ...
            'key_name'   => CustomVariables::RESERVED_OVERRIDE_HANDOFF_KEY,
            'value_type' => 'string',
            'label'      => 'Should never save',
        ], $db);
    ...
    /**
     * A datalist literally named "0" hit the same falsy-string trap as the fixed-array
     * slot 0 case, but on the read side, resolving the link back on save instead of
     * blocking a delete.
     */
    public function testDatalistNamedZeroIsNotSilentlyDetachedOnSave(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        // Load the way ordinary (non-import) code paths do, so $datalist isn't
        // pre-populated and saving has to look the link up itself.
        $loaded = DirectorProperty::loadWithUniqueId(Uuid::fromBytes($property->get('uuid')), $db);
        $loaded->set('label', 'Zero Named List Updated');
        $loaded->store();

        $reloaded = DirectorProperty::loadWithUniqueId(Uuid::fromBytes($property->get('uuid')), $db);
        $this->assertNotNull(
            $reloaded->getDatalist(),
            'Saving a property must not drop its link to a datalist literally named "0"'
        );
        $this->assertEquals($listName, $reloaded->getDatalist()->get('list_name'));
    }
    ...
    /**
     * A fixed-array's first slot always gets key_name "0", which the deletion guard used to
     * read as "nothing found" (PHP treats the string "0" as falsy), letting a datalist still
     * in use get deleted out from under it.
     */
    public function testDatalistLinkedOnlyToFixedArraySlotZeroBlocksDeletion(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $db = $this->getDb();
        $listName = self::PREFIX . 'aws_regions';
        $parentKeyName = self::PREFIX . 'deployment_info';
        ...
        $plain = (object) [
            ...
            'category'    => null,
            'description' => null,
            'items'       => ['0' => $this->datalistItemPlain('0', $parentUuid, $listName)],
        ];
        ...
        $datalist = DirectorDatalist::load($listName, $db);
        $this->expectException(Exception::class);
        $this->expectExceptionMessage('is currently being used');
        $datalist->delete();
    }
