...
class CustomVarRendererTest extends BaseTestCase
{
    ...
    public function testPrefetchFailedResetsBetweenObjects(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $db = $this->getDb();
        $hostName = self::PREFIX . 'clean-host';
        ...
        // act like an earlier object on this renderer already blew up, that
        // shouldn't stick to a fresh, healthy one
        $renderer->seedPrefetchFailed();

        $icingadbHost = new Host(['name' => $hostName]);
        $this->assertTrue($renderer->prefetchForObject($icingadbHost));

        $this->assertFalse($renderer->isPrefetchFailed());
    }

    public function testNestedLoadFailureStillMasksChild(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        try {
            $db = $this->getDb();
            $dba = $db->getDbAdapter();
            $hostName = self::PREFIX . 'partial-failure-host';
            ...
            // leftover from a previous run, deleting it cascades to its own child too
            $dba->delete('director_property', ['key_name = ?' => self::PREFIX . 'credentials']);

            // a dictionary with one sensitive child. its own top level info
            // loads before the nested lookup that would flag "token" as
            // sensitive, that nested step is what we're about to break
            $propertyUuid = Uuid::uuid4()->getBytes();
            $property = DirectorProperty::create([
                'uuid'       => $propertyUuid,
                'key_name'   => self::PREFIX . 'credentials',
            ...
            $dba->insert('icinga_host_property', [
                'property_uuid' => DbUtil::quoteBinaryCompat($propertyUuid, $dba),
                'host_uuid'     => DbUtil::quoteBinaryCompat($host->get('uuid'), $dba),
            ]);

            $renderer = new TestableCustomVarRenderer();
            $renderer->forceNestedConfigFailure();

            $icingadbHost = new Host(['name' => $hostName]);

            // Icinga DB only renders through hooks whose prefetch came back true.
            $this->assertTrue($renderer->prefetchForObject($icingadbHost));
            $this->assertTrue($writer->hasMessageContaining('forced nested config failure for test'));

            // the dictionary itself loaded fine and reads as recognized, only
            // "token" never got flagged sensitive since that's the lookup we
            // broke. masking still has to win here
            $result = $renderer->renderCustomVarValue('token', 's3cr3t-token-value', self::PREFIX . 'credentials');
    ...
    public function testSensitiveVarDoesNotLeakAcrossHosts(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $sensitiveHostName = self::PREFIX . 'creds-host';
        $plainHostName = self::PREFIX . 'plain-host';

        foreach ([$sensitiveHostName, $plainHostName] as $name) {
            if (IcingaHost::exists($name, $db)) {
                IcingaHost::load($name, $db)->delete();
            }
        }

        $sensitiveHost = IcingaHost::create([
            'object_name' => $sensitiveHostName,
            'object_type' => 'object',
        ]);
        $sensitiveHost->store($db);

        $plainHost = IcingaHost::create([
            'object_name' => $plainHostName,
            'object_type' => 'object',
        ]);
        $plainHost->store($db);

        // leftover from a previous run
        $dba->delete('director_property', ['key_name = ?' => self::PREFIX . 'api_token']);

        $propertyUuid = Uuid::uuid4()->getBytes();
        DirectorProperty::create([
            'uuid'       => $propertyUuid,
            'key_name'   => self::PREFIX . 'api_token',
            'value_type' => 'sensitive',
            'label'      => 'API Token',
        ], $db)->store();
        ...
        $dba->insert('icinga_host_property', [
            'property_uuid' => DbUtil::quoteBinaryCompat($propertyUuid, $dba),
            'host_uuid'     => DbUtil::quoteBinaryCompat($sensitiveHost->get('uuid'), $dba),
        ]);

        // one renderer instance, reused for two unrelated hosts in a row, same
        // as Icinga DB would do if it ever rendered more than one object per
        // request
        $renderer = new CustomVarRenderer();

        $icingadbSensitiveHost = new Host(['name' => $sensitiveHostName]);
        $this->assertTrue($renderer->prefetchForObject($icingadbSensitiveHost));
        $this->assertSame(
            '***',
            $renderer->renderCustomVarValue(self::PREFIX . 'api_token', 's3cr3t-token')
        );

        $icingadbPlainHost = new Host(['name' => $plainHostName]);
        $this->assertTrue($renderer->prefetchForObject($icingadbPlainHost));

        // plain-host never had this property attached, if the first host's
        // config leaked through this would come back masked instead of null
        $this->assertNull(
            $renderer->renderCustomVarValue(self::PREFIX . 'api_token', 'not-actually-secret')
        );
    }

    public function testInheritedSensitiveVarStaysMasked(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $templateName = self::PREFIX . 'inherit-template';
        $hostName = self::PREFIX . 'inherit-host';
        $propertyKey = self::PREFIX . 'inherited_secret';

        foreach ([$hostName, $templateName] as $name) {
            if (IcingaHost::exists($name, $db)) {
                IcingaHost::load($name, $db)->delete();
            }
        }
        if (DirectorProperty::exists($propertyKey, $db)) {
            DirectorProperty::load($propertyKey, $db)->delete();
        }
        ...
        DirectorProperty::create([
            ...
            'value_type' => 'sensitive',
            'label'      => 'Inherited secret',
        ], $db)->store();

        // attached only to the template, the host never has its own row in
        // icinga_host_property, it only reaches this property by inheritance
        $dba->insert('icinga_host_property', [
            'property_uuid' => DbUtil::quoteBinaryCompat($propertyUuid, $dba),
        ...
        try {
            ...
            $this->assertSame(
                '***',
                $renderer->renderCustomVarValue($propertyKey, 's3cr3t-inherited-value'),
                'A sensitive property attached only to an ancestor template must still '
                . 'mask for a host that inherits it without its own attachment row'
            );
        } finally {
            $host->delete();
            $template->delete();
