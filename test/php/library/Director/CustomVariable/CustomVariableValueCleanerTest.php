...
class CustomVariableValueCleanerTest extends BaseTestCase
{
    ...
    public function testMigrationDoesNotTouchAnUnrelatedHostAlreadyUsingTheDestinationName(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $renamedHost = IcingaHost::create([
            'object_name' => self::PREFIX . 'migration_renamed_host',
            'object_type' => 'object',
            'address'     => '192.0.2.86',
        ], $db);
        $renamedHost->store();

        $unrelatedHost = IcingaHost::create([
            'object_name' => self::PREFIX . 'migration_unrelated_host',
            'object_type' => 'object',
            'address'     => '192.0.2.87',
        ], $db);
        $unrelatedHost->store();
        ...
        $dba->insert('icinga_host_var', [
            'host_id'  => $renamedHost->get('id'),
            'varname'  => $oldKeyName,
            'varvalue' => json_encode('secops-oncall@example.com'),
            'format'   => 'json',
        ]);

        // this host never had the old name, it has nothing to do with the rename
        $dba->insert('icinga_host_var', [
            'host_id'  => $unrelatedHost->get('id'),
            'varname'  => $newKeyName,
            'varvalue' => json_encode('platform-team-oncall@example.com'),
            'format'   => 'json',
        ]);

        // a retype that can't keep the value, e.g. moving from string to number
        $migration = new PropertyValueMigration(
            oldVarname: $oldKeyName,
            newVarname: $newKeyName,
            oldRootType: 'string',
        ...
        (new CustomVariableValueCleaner($db))->applyValueMigration($migration);

        $renamedHostValue = $dba->fetchOne(
            $dba->select()->from('icinga_host_var', ['varvalue'])
                ->where('host_id = ?', $renamedHost->get('id'))
                ->where('varname = ?', $newKeyName)
        );
        $unrelatedHostValue = $dba->fetchOne(
            $dba->select()->from('icinga_host_var', ['varvalue'])
                ->where('host_id = ?', $unrelatedHost->get('id'))
                ->where('varname = ?', $newKeyName)
        );

        $this->assertFalse(
            $renamedHostValue,
            'a value that could not survive the retype must be gone from the renamed host'
        );
        $this->assertEquals(
            json_encode('platform-team-oncall@example.com'),
            $unrelatedHostValue,
            'a host that was never part of the rename must keep its own value under that name'
        );
    }
    ...
    protected function tearDown(): void
    {
        if ($this->hasDb()) {
            ...
            $dba->delete('icinga_host', ['object_name = ?' => self::PREFIX . 'migration_renamed_host']);
            $dba->delete('icinga_host', ['object_name = ?' => self::PREFIX . 'migration_unrelated_host']);
