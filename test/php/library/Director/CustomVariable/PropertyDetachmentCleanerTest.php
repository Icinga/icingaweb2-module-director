...
class PropertyDetachmentCleanerTest extends BaseTestCase
{
    ...
    public function testPreviewFlagsAValueOnADescendantWhenAMiddleTemplateDropsTheImport(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        // leaf inherits REGION_KEY through mid, then saves its own value
        (new CustomVariableValueApplier($db))->apply(new CustomVarApplyRequest(
            $leaf,
            [self::REGION_KEY => 'eu-west'],
            'index',
        ...
        $atRisk = PropertyDetachmentCleaner::previewCustomVarsLostIfImportsRemoved($mid, []);

        $this->assertEquals(
            [self::REGION_KEY],
            $atRisk,
            'dropping an import on a middle template must flag a descendant\'s own saved value too'
        );
    }

    public function testPreviewStaysEmptyWhenADescendantReachesTheValueAnotherWay(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $leaf = $this->createLeafHost($db);
        // leaf reaches the property both through mid and directly via templateTwo
        $leaf->setImports([self::MID_TEMPLATE_NAME, self::TEMPLATE_TWO_NAME]);
        $leaf->store($db);
        $leaf = IcingaHost::load(self::LEAF_HOST_NAME, $db);
        ...
        $this->assertEquals(
            [],
            $atRisk,
            'a descendant still reaching the property through another import must not be flagged'
        );
    }

    /**
     * A descendant importing the exact same template both through the
     * middle template and directly on its own is a rare setup, and the
     * preview isn't precise enough to see the direct import still saves
     * it. Over-warning here is fine, this locks that tradeoff in on
     * purpose so nobody "fixes" it into a false negative later.
     */
    public function testPreviewOverWarnsWhenDescendantAlsoImportsTheDroppedTemplateDirectly(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $leaf = $this->createLeafHost($db);
        // leaf imports templateOne both through mid and on its own
        $leaf->setImports([self::MID_TEMPLATE_NAME, self::TEMPLATE_ONE_NAME]);
        $leaf->store($db);
        $leaf = IcingaHost::load(self::LEAF_HOST_NAME, $db);
        ...
        $this->assertEquals(
            [self::REGION_KEY],
            $atRisk,
            'a value safe through a direct duplicate import still gets flagged, known and accepted tradeoff'
        );
    }
