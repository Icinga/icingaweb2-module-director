...
class CustomVariableValueApplierTest extends BaseTestCase
{
    ...
    private const DATALIST_KEY = self::PREFIX . 'applier_env_choice';
    private const DATALIST_NAME = self::PREFIX . 'applier_env_list';
    ...
    public function testNullClearsAnAttachedDictionaryValue(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        // a second POST clearing that value used to fail validation instead, the
        // leftover deleted entry from the first step still looked like a real
        ...
        $this->assertNull(
            ...
            'a null value must clear an existing structured value instead of failing validation'
        );
        ...
        $this->assertEquals(
            2,
            (int) $count,
            'clearing a local value through a POST must not detach the property itself'
        );
    }

    public function testReservedOverrideHandoffKeyCanNotBeSetThroughTheRestApi(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        (new CustomVariableValueApplier($db))->apply(new CustomVarApplyRequest(
            $host,
            [CustomVariables::RESERVED_OVERRIDE_HANDOFF_KEY => 'evil'],
            'variables',
            'PUT',
            false
        ));
    ...
    public function testVariablesEndpointRejectsADatalistValueRestrictedToAnotherRole(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $db = $this->getDb();
        $host = $this->createTemplate($db);
        $this->createDatalistStrictProperty($db, ['prod' => ['security-team'], 'dev' => null]);
        $this->setCurrentUserRoles(['network-team']);

        // an ordinary vars write, not the dedicated datalist entries endpoint,
        // must still respect the role restriction on the entry
        $this->expectException(InvalidArgumentException::class);
        (new CustomVariableValueApplier($db))->apply(new CustomVarApplyRequest(
            $host,
            [self::DATALIST_KEY => 'prod'],
            'variables',
            'PUT',
            false
        ));
    ...
    private function createDatalistStrictProperty($db, array $allowedRolesByEntry): DirectorProperty
    {
        ...
        $datalist->setEntries(array_map(
            fn ($name) => (object) [
                ...
                'allowed_roles' => $allowedRolesByEntry[$name],
            ],
            array_keys($allowedRolesByEntry)
        ));
        $datalist->store();
    ...
    {
        $user = new User(self::PREFIX . 'applier-user');
    ...
    protected function tearDown(): void
    {
        if ($this->hasDb()) {
            ...
            $dba->delete(
                'director_property',
                $dba->quoteInto('key_name IN (?)', [self::ENV_KEY, self::MYSQL_KEY, self::DATALIST_KEY])
            );
