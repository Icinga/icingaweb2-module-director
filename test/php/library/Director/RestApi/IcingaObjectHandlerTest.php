...
class IcingaObjectHandlerTest extends BaseTestCase
{
    ...
    private const API_USER_NAME = self::PREFIX . 'automation';
    private const HOSTGROUP_TEMPLATE_NAME = self::PREFIX . 'hostgroup-template';
    private const HOSTGROUP_LEAF_NAME = self::PREFIX . 'hostgroup-leaf';
    ...
    public function testPutOnAnApiUserWorksEvenThoughItHasNoImportsSupport(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        if (IcingaApiUser::exists(self::API_USER_NAME, $db)) {
            IcingaApiUser::load(self::API_USER_NAME, $db)->delete();
        }

        $user = IcingaApiUser::create([
            'object_name' => self::API_USER_NAME,
            'object_type' => 'object',
            'password'    => 'first-password',
        ]);
        $user->store($db);
        $user = IcingaApiUser::load(self::API_USER_NAME, $db);
        ...
        $reloaded = IcingaApiUser::load(self::API_USER_NAME, $db);
        $this->assertEquals('rotated', $reloaded->get('password'));
    }

    public function testPutDroppingAnImportOnAHostGroupDoesNotBlowUp(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        if (IcingaHostGroup::exists(self::HOSTGROUP_LEAF_NAME, $db)) {
            IcingaHostGroup::load(self::HOSTGROUP_LEAF_NAME, $db)->delete();
        }

        if (IcingaHostGroup::exists(self::HOSTGROUP_TEMPLATE_NAME, $db)) {
            IcingaHostGroup::load(self::HOSTGROUP_TEMPLATE_NAME, $db)->delete();
        }

        $template = IcingaHostGroup::create([
            'object_name' => self::HOSTGROUP_TEMPLATE_NAME,
            'object_type' => 'template',
        ]);
        $template->store($db);

        $leaf = IcingaHostGroup::create([
            'object_name' => self::HOSTGROUP_LEAF_NAME,
            'object_type' => 'object',
        ]);
        $leaf->setImports([self::HOSTGROUP_TEMPLATE_NAME]);
        $leaf->store($db);
        $leaf = IcingaHostGroup::load(self::HOSTGROUP_LEAF_NAME, $db);

        $handler = new IcingaObjectHandler(new Request(), new Response(), $db);
        $method = new ReflectionMethod($handler, 'persistObjectAndApplyVars');

        // hostgroups support imports but not custom vars, dropping an
        // import here used to crash looking up a property table that
        ...
        $reloaded = IcingaHostGroup::load(self::HOSTGROUP_LEAF_NAME, $db);
        $this->assertEquals([], $reloaded->getImports());
    }
    ...
    public function testServiceVariablesWriteReloadsTheRightServiceWhenNamesCollideAcrossHosts(): void
    {
        ...
        $this->assertNull(
            $reloadedB->vars()->get(self::SSH_PORT_KEY),
