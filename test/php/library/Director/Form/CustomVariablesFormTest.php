...
{
    private const DATALIST_PREFIX = '___TEST___';
    private const DATALIST_HOST_NAME = self::DATALIST_PREFIX . 'cv-form-host';
    private const DATALIST_KEY = self::DATALIST_PREFIX . 'cv_form_env_choice';
    private const DATALIST_NAME = self::DATALIST_PREFIX . 'cv_form_env_list';
    ...
    public function testDatalistStrictRejectsAChangedValueNotInTheAllowedList(): void
    {
        ...
        $property = $this->createDatalistStrictProperty($db, ['dev']);
        ...
        $form->registerElement(new TestablePropertiesDictionary([self::DATALIST_KEY => 'prod']));

        // a hidden form field can be tampered with to skip the widget's own
        // validator, this must still get caught here
        $this->expectException(InvalidArgumentException::class);
        self::callMethod($form, 'persistPropertyChanges', []);
    }

    public function testDatalistStrictAllowsAChangedValueInTheAllowedList(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $db = $this->getDb();
        $host = $this->createDatalistTestHost($db);
        $property = $this->createDatalistStrictProperty($db, ['dev', 'prod']);
        ...
        $this->assertSame('prod', $host->vars()->get(self::DATALIST_KEY)->getValue());
    }

    public function testDatalistStrictSkipsRevalidationWhenValueIsUnchanged(): void
    {
        if ($this->skipForMissingDb()) {
            return;
        }
        ...
        $db = $this->getDb();
        $host = $this->createDatalistTestHost($db);
        // the datalist only allows "dev" now, "prod" isn't a valid entry
        // any more, say someone removed it after this value was saved
        $property = $this->createDatalistStrictProperty($db, ['dev']);
        ...
        // resubmits the very same value, this field was never touched
        $form->registerElement(new TestablePropertiesDictionary([self::DATALIST_KEY => 'prod']));

        // must not throw, an old value that's no longer valid shouldn't block
        // someone from editing an unrelated field on the same object
        self::callMethod($form, 'persistPropertyChanges', []);

        $this->assertSame('prod', $host->vars()->get(self::DATALIST_KEY)->getValue());
    }

    private function createDatalistTestHost($db): IcingaHost
    {
        if (IcingaHost::exists(self::DATALIST_HOST_NAME, $db)) {
            IcingaHost::load(self::DATALIST_HOST_NAME, $db)->delete();
        }

        $host = IcingaHost::create([
            'object_name' => self::DATALIST_HOST_NAME,
            'object_type' => 'template',
        ]);
        $host->store($db);
    ...
    private function createDatalistStrictProperty($db, array $allowedEntryNames): DirectorProperty
    {
        $datalist = DirectorDatalist::create(['list_name' => self::DATALIST_NAME, 'owner' => 'test'], $db);
    ...
    protected function tearDown(): void
    {
        if ($this->hasDb()) {
            ...
            $dba->delete('director_property', $dba->quoteInto('key_name = ?', self::DATALIST_KEY));
            $dba->delete('director_datalist', $dba->quoteInto('list_name = ?', self::DATALIST_NAME));
        }
