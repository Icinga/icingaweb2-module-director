...
class DictionaryItemTest extends BaseTestCase
{
    ...
    public function testTamperedHiddenTypeCannotDowngradeAStrictDatalistToAPlainTextField(): void
    {
        ...
        $property = $this->createDatalistStrictProperty(['dev']);
        $propertyData = [
            'uuid' => $property->get('uuid'),
            'key_name' => self::PREFIX . 'environment_choice',
        ...
        // the hidden 'type' field gets repopulated from a submitted request before
        // assemble() runs, a tampered submission could claim 'string' here to make
        // assemble() build a free text field instead of the restricted select
        $item->populate(array_merge(DictionaryItem::prepare($propertyData), ['type' => 'string']));
        $item->ensureAssembled();

        $this->assertInstanceOf(SelectElement::class, $item->getElement('var'));

        // "prod" was never added to the datalist, only the real select would still
        // reject it
        $item->getElement('var')->setValue('prod');
        $this->assertFalse($item->getElement('var')->validate()->isValid());
    }
    ...
    protected function tearDown(): void
    {
        if ($this->hasDb()) {
            ...
            foreach ($this->createdDatalistNames as $listName) {
                $dba->delete('director_datalist', $dba->quoteInto('list_name = ?', $listName));
            }
        }
