...
class TestableDictionaryItem extends DictionaryItem
{
    ...
    public function setTestConfig(array $config): void
    {
        ...
        // getItem() now trusts $fields for the type, same as the real assemble(), so
        // keep it in sync instead of only feeding the hidden element below.
        $this->fields = array_merge($this->fields, ['value_type' => $config['type'] ?? '']);
    }
