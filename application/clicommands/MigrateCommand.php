...
class MigrateCommand extends Command
{
    ...
    public function datafieldsAction()
    {
        ...
        foreach ($this->existingCustomProperties as $varname) {
            ...
            if ($this->isVerbose) {
                echo "[-] Skipping migrating datafield '$varname' as a custom property"
    ...
    private function migrateDatafields(array $customProperties, bool $dryRun): void
    {
        ...
        foreach ($customProperties as $varName => $customProperty) {
            ...
            if (! $dryRun) {
                ...
                $propertyUuid = Uuid::fromBytes($customProperty['uuid']);
    ...
    private function checkUnmigrateableDatafieldTypes(): void
    {
        ...
        if ($total > 0) {
            printf(
                ...
                . " with new custom property support: %d\n\n",
                $total
            );
        }
    }
    ...
    private function migrateDatafieldObjectTemplateBinding(int $datafieldIdId, UuidInterface $propertyUuid): void
    {
        $db = $this->db();
        $objectTypes = ['host', 'service', 'notification', 'command', 'user'];
        foreach ($objectTypes as $type) {
            $query = $db->getDbAdapter()->select()->from(['io' => "icinga_{$type}"], ['*'])
                ->join(['iof' => "icinga_{$type}_field"], "io.id = iof.{$type}_id", [])
                ->where('iof.datafield_id = ?', $datafieldIdId);
            ...
            foreach ($objects as $object) {
                $db->insert(
                    ...
                    ['property_uuid' => $propertyUuid->getBytes(), "{$type}_uuid" => $object->get('uuid')]
                );
            }
        }
    }
