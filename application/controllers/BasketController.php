...
class BasketController extends ActionController
{
    ...
    private function describeCustomPropertyMigrationChanges(PropertyValueMigration $migration, BasketDiff $diff)
    {
        ...
        if ($migration->retyped) {
            $lines[] = sprintf(
                $this->translate('Type is changing, stored values under "%s" that no longer fit will be cleared'),
                $migration->oldVarname
            );
    ...
    private function collectCustomPropertyChangeLines(array $changes, array &$lines, string $prefix = ''): void
    {
        foreach ($changes as $change) {
            ...
            } elseif ($change->retyped) {
                $lines[] = sprintf(
                    $this->translate('"%s" type is changing, its stored value will be cleared if it no longer fits'),
                    $path
                );
            ...
            if (! $change->retyped && ! empty($change->children)) {
