...
class DeletePropertyForm extends CompatForm
{
    ...
    /**
     * Fetch property items for the given UUID
     *
     * @param UuidInterface $uuid UUID of the given property
     *
     * @return array
     */
    private function fetchPropertyItems(UuidInterface $uuid): array
    {
        $db = $this->db->getDbAdapter();
    ...
    protected function onSuccess(): void
    {
        ...
        if ($this->parent) {
            ...
            $parentProp = [];
            if ($rootProp['parent_uuid'] !== null) {
                $rootUuid = Uuid::fromBytes($rootProp['parent_uuid']);
                $parentProp = $rootProp;
                $rootProp = $this->fetchProperty($rootUuid);
                    ...
                    foreach ($varValue as $key => $value) {
                        if (empty($parentProp)) {
                            ...
                            $this->removeDictionaryItem(
                                $value,
                                [$parentProp['key_name'], $prop['key_name']]
                            );
                        }
                    ...
                    if ($parentProp && $parentProp['value_type'] === 'fixed-array') {
                        ...
                        foreach ($propItems as $propItem) {
                            $this->db->insert('director_property', [
                                ...
                                'parent_uuid' => $directParentUuid->getBytes(),
                                'key_name' => $count,
                        ...
                        $varValue[$parentProp['key_name']] = array_values($varValue[$parentProp['key_name']]);
                    } elseif ($rootProp['value_type'] === 'fixed-array') {
                        $propItems = $this->fetchPropertyItems($rootUuid);;
                        $this->db->delete('director_property', Filter::where('parent_uuid', $rootUuid->getBytes()));
