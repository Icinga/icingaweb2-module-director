...
class DictionaryItem extends FieldsetElement
{
    ...
    protected function assemble(): void
    {
        ...
        $valElementName = 'var';
        // Read the type off the db-fetched fields, not the hidden field. A submitted
        // request repopulates that field before assemble() runs, so a tampered value
        // could downgrade a strict datalist to a plain string and skip its validator.
        $type = $this->fields['value_type'] ?? '';
        ...
        if ($this->fields['required'] ?? false) {
            ...
            $isInherited = ($type === 'fixed-array' || $type === 'fixed-dictionary')
                ...
                : ! CustomVariablesForm::isValueUnset($inherited);
    ...
    {
        $value = $this->ensureAssembled()->getElement('inherited')->getValue();

        return ! CustomVariablesForm::isValueUnset($value);
    }
    ...
    public function populate($values)
    {
        ...
        if (
            ($this->fields['value_type'] ?? null) === 'datalist-non-strict'
    ...
    public function getItem(bool $applyUnchangedDefaults = true): array
    {
        ...
        if ($itemValue instanceof NestedDictionary or $itemValue instanceof Dictionary) {
            ...
            if (! $isUntouched) {
                ...
                if (($this->fields['value_type'] ?? '') === 'fixed-array') {
                    $value = $values['value'];
                    ksort($value);
            ...
            ($this->fields['value_type'] ?? '') === 'datalist-non-strict'
            && self::fetchItemType(Uuid::fromBytes($this->fields['uuid'])) === 'string'
        ) {
            ...
            $inheritedValue = $this->getElement('inherited')->getValue();

            if (empty($parentType) && ! CustomVariablesForm::isValueUnset($inheritedValue)) {
