...
class HostServiceBlacklistForm extends CompatForm
{
    ...
    protected function blacklist(): int
    {
        ...
        // both writes have to land together, otherwise the service stays active
        // while its override values are already gone
        $db->beginTransaction();

        try {
            $this->host->unsetOverriddenServiceVars($this->service->getObjectName())->store();

            $inserted = $db->insert('icinga_host_service_blacklist', [
