...
class CustomVariableForm extends CompatForm
{
     ...
     *
     * True once the key name changed, the property is really still used
    ...
    public function isPendingRenameConfirmation(): bool
    {
        ...
        return $this->isCurrentlyUsed(new CustomVariableValueCleaner($this->db));
    }
    ...
    protected function assemble(): void
    {
        ...
        if ($this->hideKeyNameElement) {
            ...
            $keyNameValidators[] = new CallbackValidator(function ($value, $validator) {
                if ((string) $value !== CustomVariables::RESERVED_OVERRIDE_HANDOFF_KEY) {
                    return true;
                }

                $validator->addMessage($this->translate(
                    'This name is reserved for Director\'s internal use and cannot be used as a'
                    . ' property key.'
                ));
        ...
        if ($type === 'dynamic-array') {
            ...
            $this->getElement('value_type')->getAttributes()->set('strict', $isStrict);
            $this->addElement(
                'select',
                'list',
                [
    ...
    protected function onSuccess(): void
    {
        ...
        // The form validator should already catch this, this is just a backstop since
        // this method writes straight to the DB and never goes through DirectorProperty.
        if (($values['key_name'] ?? null) === CustomVariables::RESERVED_OVERRIDE_HANDOFF_KEY) {
            throw new InvalidArgumentException(sprintf(
                "'%s' is reserved for Director's internal use, it can't be used as a property name",
