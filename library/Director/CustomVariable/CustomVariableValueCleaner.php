...
class CustomVariableValueCleaner
{
     ...
     * @param ?array     $onlyIdsByType Only touch these holder ids per object type. Leave this
     *                                  out to hit every row under that name, pass it when only
     *                                  some of those rows actually just changed and the rest
     *                                  need to be left alone.
     *
     * @return void
     */
    private function forEachMatchingVarRow(string $varname, callable $transform, ?array $onlyIdsByType = null): void
    {
        $db = $this->db->getDbAdapter();
        ...
        foreach (self::OBJECT_TYPES as $objectType) {
            ...
            if ($onlyIdsByType !== null) {
                $ids = $onlyIdsByType[$objectType] ?? [];
            ...
            if ($onlyIdsByType !== null) {
                $select->where("{$idColumn} IN (?)", $ids);
            }

            $varRows = $db->fetchAll($select, [], Zend_Db::FETCH_ASSOC);
     ...
     *
     * @return array{renamedIdsByType: array<string, int[]>, collisions: int} Which holder ids
     *         actually got renamed per object type, and how many were left under the old
     *         varname because the new one was already taken there
     */
    private function renameRootVarnamePerHolder(string $oldVarname, string $newVarname): array
    {
        $db = $this->db->getDbAdapter();
        $renamedIdsByType = [];
        $collisions = 0;
        ...
        foreach (self::OBJECT_TYPES as $objectType) {
            ...
            if (empty($destIds)) {
                ...
                $renamedIdsByType[$objectType] = $sourceIds;
            ...
            $renamedIdsByType[$objectType] = $safeIds;
        }

        return ['renamedIdsByType' => $renamedIdsByType, 'collisions' => $collisions];
    }
        ...
        foreach ($rows as $row) {
            ...
            foreach ($overrideVars as $serviceName => $serviceVars) {
                ...
                if ($pathWithinRootValue === null) {
                    ...
                    // a fixed-array's positions have to stay 0, 1, 2..., otherwise it
                    // encodes as a JSON object once a slot in the middle is gone
                    if (! $preserveIndex && $parent['value_type'] === 'fixed-array') {
                        $serviceVars[$rootKeyName] = array_values($serviceVars[$rootKeyName]);
                    }
    ...
    public function renameStoredValues(string $oldVarname, string $newVarname): int
    {
        ...
        return $this->renameRootVarnamePerHolder($oldVarname, $newVarname)['collisions'];
    }
    ...
    public function applyValueMigration(PropertyValueMigration $migration): int
    {
        ...
        if ($migration->oldVarname !== $migration->newVarname) {
            $renamed = $this->renameRootVarnamePerHolder($migration->oldVarname, $migration->newVarname);

            if ($renamed['collisions'] > 0) {
                $rebuilder->noteRootConflict($renamed['collisions']);
            }

            // only rebuild the holders we actually just renamed, a holder that already
            // sat under the new name never moved and must not get dragged into this
            $this->forEachMatchingVarRow(
                $migration->newVarname,
                fn($decoded) => $rebuilder->rebuildRootValue($decoded, $migration),
                $renamed['renamedIdsByType']
            );
        } else {
            // no rename, just a type change, this really does apply to every holder
            $this->forEachMatchingVarRow(
                $migration->newVarname,
                fn($decoded) => $rebuilder->rebuildRootValue($decoded, $migration)
            );
