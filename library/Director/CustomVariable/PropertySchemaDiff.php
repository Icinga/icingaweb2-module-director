...
class PropertySchemaDiff
{
    ...
    public function diff(DirectorProperty $root): PropertyValueMigration
    {
        ...
        if ($retyped) {
            // The type changed, so the old value only survives if it still fits the
            // new one, checked later against what's actually stored. Whatever changed
            // underneath doesn't matter anymore either way, it's carried by the root.
            return new PropertyValueMigration(
                oldVarname: $oldVarname,
                newVarname: $newVarname,
                oldRootType: $oldRootType,
