...
class CustomVariables implements Iterator, Countable, IcingaConfigRenderer
{
    ...
    public static function loadForStoredObject(IcingaObject $object)
    {
        ...
        foreach ($db->fetchAll($query) as $row) {
            if ($row->varname === self::RESERVED_OVERRIDE_HANDOFF_KEY) {
                Logger::warning(
                    "Director: found a stored custom variable named '%s', which is reserved for"
                    . ' internal use. Dropping it to avoid corrupting apply-for rendering.',
                    self::RESERVED_OVERRIDE_HANDOFF_KEY
                );
