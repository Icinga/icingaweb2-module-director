...
class CustomVariableValueValidator
{
     ...
     * Entries the current user can't see because of their allowed_roles
     * setting don't count as valid values either.
     *
     * @param string $key
     * @param mixed $value
     * @param UuidInterface $propertyUuid
    ...
    public static function assertDatalistValueAllowed(
        ...
        foreach ($datalist->getEntries() as $entry) {
            // get() would json_decode this, isAllowedForCurrentUser() wants the raw column value
            if (! DirectorDatalistEntry::isAllowedForCurrentUser($entry->getProperty('allowed_roles'))) {
                continue;
            }
