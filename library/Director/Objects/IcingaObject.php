...
abstract class IcingaObject extends DbObject implements IcingaConfigRenderer
{
        ...
        foreach ($objects as $name => $object) {
            ...
            foreach ($object->$getInherited() as $key => $value) {
                ...
                if (
                    ...
                    // this only carries what came down one branch of the inheritance
                    // chain, another branch may have already added its own entries to
                    // the dictionary we're building, so add to it instead of overwriting
                    ...
                    // more than one template might get credited for this entry, so check
                    // name by name instead of comparing the whole list at once, or a
                    // template already listed could get added again
                    $vals['_ORIGINS_']->$key = $this->mergeVarOrigins(
                        $vals['_ORIGINS_']->$key ?? null,
                        $origins->$key
                    );
    ...
    /**
     * Combine two comma-separated origin lists without repeating a name
     *
     ...
     * @return string
     */
    protected function mergeVarOrigins($existing, $incoming)
    {
        if ($existing === null) {
            return $incoming;
        }
        ...
        foreach (explode(', ', $incoming) as $name) {
